...
def posicoes_rotulos_maior_menor(*series):
    """
    Para cada ponto (mesmo índice), compara os valores entre as séries fornecidas e define:
      - maior valor -> 'top center'
      - menor valor -> 'bottom center'
    Para pontos com <2 valores válidos (NaN/None), mantém 'top center'.
    Empates: todos ficam 'top center' naquele ponto.
    Retorna uma lista de listas (uma lista de textposition para cada série).
    """
    if not series:
        return []
    ...
    for s in series:
        if isinstance(s, (pd.Series,)):
            ser_list.append(list(s.values))
        else:
            ser_list.append(list(s))

    n = max(len(s) for s in ser_list) if ser_list else 0
    # Normaliza tamanhos (preenche com NaN)
    for i in range(len(ser_list)):
        if len(ser_list[i]) < n:
            ser_list[i] = ser_list[i] + [float("nan")] * (n - len(ser_list[i]))

    pos_all = [["top center"] * n for _ in ser_list]

    for idx in range(n):
        vals = []
        for j, s in enumerate(ser_list):
            v = s[idx]
            try:
                is_valid = v is not None and not (isinstance(v, float) and pd.isna(v)) and not pd.isna(v)
            except Exception:
                is_valid = v is not None
            if is_valid:
                vals.append((j, float(v)))
        if len(vals) < 2:
            continue
        ...
        # Se tudo igual, mantém topo
        if v_max == v_min:
            continue

        # Marca maior(es) e menor(es)
        for j, v in vals:
            if v == v_max:
                pos_all[j][idx] = "top center"
            elif v == v_min:
                pos_all[j][idx] = "bottom center"

    return pos_all


def textpositions_por_primeiro_simulado(*series_y, default="top center"):
    """
    Para um conjunto de séries Y (traces) do MESMO gráfico, compara apenas o ponto do
    '1º Simulado' (índice 0). O maior valor fica com rótulo em 'top center' e o menor
    em 'bottom center'. As demais séries (se houver) ficam em 'top center'.
    ...
    n = len(ys[0])
    # garante comprimentos consistentes
    for y in ys:
        if len(y) != n:
            raise ValueError("Séries com comprimentos diferentes no mesmo gráfico.")

    # posições padrão
    pos_all = [[default] * n for _ in ys]

    if n == 0 or len(ys) < 2:
        return pos_all

    # compara apenas o primeiro ponto
    first_vals = []
    for idx, y in enumerate(ys):
        v = y[0]
        if v is None or (isinstance(v, float) and pd.isna(v)) or pd.isna(v):
            first_vals.append((idx, None))
        else:
            first_vals.append((idx, float(v)))

    valid = [(i, v) for i, v in first_vals if v is not None]
    if len(valid) < 2:
        return pos_all
    ...
    # se todos iguais, mantém padrão
    if max_val == min_val:
        return pos_all

    for i, v in valid:
        if v == max_val:
            pos_all[i][0] = "top center"
        elif v == min_val:
            pos_all[i][0] = "bottom center"
        else:
            pos_all[i][0] = default
...
if aba == "Redação: Médias e Participações por Escola":
    ...
    # Ajuste de rótulos: em cada etapa, maior valor acima e menor valor abaixo
    pos_notas, pos_part = posicoes_rotulos_maior_menor(notas_norm, part_frac)

    # Posição dos rótulos no 1º Simulado (maior em cima, menor em baixo)
    pos_nota, pos_part = textpositions_por_primeiro_simulado(notas_norm, part_frac)
...
# ===============================================================
    ...
    # Ajuste de rótulos (por etapa): Escola vs Regional
    if notas_reg is not None:
        _pos = posicoes_rotulos_maior_menor(notas_norm, (notas_reg / 1000.0))
        pos_escola_med, pos_reg_med = _pos[0], _pos[1]
    else:
        pos_escola_med = ["top center"] * len(etapas_red)
        pos_reg_med = ["top center"] * len(etapas_red)

    # Posição dos rótulos no 1º Simulado (maior em cima, menor em baixo)
    pos_med_escola = ["top center"] * len(etapas_red)
    pos_med_reg    = ["top center"] * len(etapas_red)
    if notas_reg is not None:
        pos_med_escola, pos_med_reg = textpositions_por_primeiro_simulado(notas_norm, (notas_reg / 1000.0))
    ...
    # Ajuste de rótulos (por etapa): Participação Escola vs Regional
    if part_reg is not None:
        _pos = posicoes_rotulos_maior_menor(part_frac, (part_reg / 100.0))
        pos_escola_part, pos_reg_part = _pos[0], _pos[1]
    else:
        pos_escola_part = ["top center"] * len(etapas_red)
        pos_reg_part = ["top center"] * len(etapas_red)

    # Posição dos rótulos no 1º Simulado (maior em cima, menor em baixo)
    pos_part_escola = ["top center"] * len(etapas_red)
    pos_part_reg    = ["top center"] * len(etapas_red)
    if part_reg is not None:
        pos_part_escola, pos_part_reg = textpositions_por_primeiro_simulado(part_frac, (part_reg / 100.0))
...
# ===============================================================
    ...
    # Ajuste de rótulos (por etapa): Acertos vs Participação
    pos_acertos, pos_part_obj = posicoes_rotulos_maior_menor(acertos_obj_frac, part_obj_frac)

    # Posição dos rótulos no 1º Simulado (maior em cima, menor em baixo)
    pos_acertos, pos_part = textpositions_por_primeiro_simulado(acertos_obj_frac, part_obj_frac)
...
# ===============================================================
    ...
    # Ajuste de rótulos (por etapa): Acertos Escola vs Regional
    if acertos_obj_reg is not None:
        _pos = posicoes_rotulos_maior_menor(acertos_obj_frac, (acertos_obj_reg / 100.0))
        pos_escola_ac, pos_reg_ac = _pos[0], _pos[1]
    else:
        pos_escola_ac = ["top center"] * len(etapas_obj)
        pos_reg_ac = ["top center"] * len(etapas_obj)

    # Posição dos rótulos no 1º Simulado (maior em cima, menor em baixo)
    pos_acertos_escola = ["top center"] * len(etapas_obj)
    pos_acertos_reg    = ["top center"] * len(etapas_obj)
    if acertos_obj_reg is not None:
        pos_acertos_escola, pos_acertos_reg = textpositions_por_primeiro_simulado(acertos_obj_frac, (acertos_obj_reg / 100.0))
    ...
    if part_obj_reg is not None:
        _pos = posicoes_rotulos_maior_menor(part_obj_frac, (part_obj_reg / 100.0))
        pos_escola_po, pos_reg_po = _pos[0], _pos[1]
    else:
        pos_escola_po = ["top center"] * len(etapas_obj)
        pos_reg_po = ["top center"] * len(etapas_obj)

    # Posição dos rótulos no 1º Simulado (maior em cima, menor em baixo)
    pos_part_escola = ["top center"] * len(etapas_obj)
    pos_part_reg    = ["top center"] * len(etapas_obj)
    if part_obj_reg is not None:
        pos_part_escola, pos_part_reg = textpositions_por_primeiro_simulado(part_obj_frac, (part_obj_reg / 100.0))
