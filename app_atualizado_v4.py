...
def hover_text_com_variacao(etapas, valores_num, fmt_valor, nome_valor="Valor"):
    """
    Cria uma lista de textos de hover com:
      - Etapa
      - Valor (formatado)
      - Variação Absoluta e Variação Percentual (a partir do 2º ponto)

    Regras:
      - Na 1ª avaliação: não mostra variações
      - Se não for possível calcular (NaN/prev=0): mostra '-'
    """
    # garante lista
    if isinstance(valores_num, pd.Series):
        valores = valores_num.tolist()
    else:
        valores = list(valores_num)

    hover = []
    prev = None
    for i, (etapa, v) in enumerate(zip(etapas, valores)):
        if v is None or (isinstance(v, float) and pd.isna(v)) or pd.isna(v):
            val_str = "-"
            v_num = None
        else:
            v_num = float(v)
            val_str = fmt_valor(v_num)

        if i == 0:
            hover.append(f"Etapa: {etapa}<br>{nome_valor}: {val_str}")
        else:
            if v_num is not None and prev is not None:
                var_abs = v_num - prev
                var_abs_str = fmt_valor(var_abs)
                if prev != 0:
            ...
            hover.append(
                f"Etapa: {etapa}<br>{nome_valor}: {val_str}"
                f"<br>Variação Absoluta: {var_abs_str}"
                f"<br>Variação Percentual: {var_pct_str}"
            )

        prev = v_num if v_num is not None else prev
...
if aba == "Redação: Médias e Participações por Escola":
    ...
    hover_nota = hover_text_com_variacao(etapas_red, notas, lambda x: f"{x:.2f}".replace('.', ','), nome_valor="Nota")
    hover_part = hover_text_com_variacao(etapas_red, part, lambda x: f"{x:.2f}%".replace('.', ','), nome_valor="Participação")
...
# ===============================================================
    ...
    hover_med_escola = hover_text_com_variacao(etapas_red, notas, lambda x: f"{x:.2f}".replace('.', ','), nome_valor="Média")
    hover_part_escola = hover_text_com_variacao(etapas_red, part, lambda x: f"{x:.2f}%".replace('.', ','), nome_valor="Participação")

    hover_med_reg = None
    hover_part_reg = None
    if notas_reg is not None:
        hover_med_reg = hover_text_com_variacao(etapas_red, notas_reg, lambda x: f"{x:.2f}".replace('.', ','), nome_valor="Média")
    if part_reg is not None:
        hover_part_reg = hover_text_com_variacao(etapas_red, part_reg, lambda x: f"{x:.2f}%".replace('.', ','), nome_valor="Participação")
...
# ===============================================================
    ...
    hover_acertos = hover_text_com_variacao(etapas_obj, acertos_obj, lambda x: f"{x:.2f}%".replace('.', ','), nome_valor="Acertos")
    hover_part_obj = hover_text_com_variacao(etapas_obj, part_obj, lambda x: f"{x:.2f}%".replace('.', ','), nome_valor="Participação")
...
# ===============================================================
    ...
    hover_acertos_escola = hover_text_com_variacao(etapas_obj, acertos_obj, lambda x: f"{x:.2f}%".replace(".", ","), nome_valor="Acertos")
    hover_part_escola_obj = hover_text_com_variacao(etapas_obj, part_obj, lambda x: f"{x:.2f}%".replace(".", ","), nome_valor="Participação")

    hover_acertos_reg = None
    hover_part_reg_obj = None
    if acertos_obj_reg is not None:
        hover_acertos_reg = hover_text_com_variacao(etapas_obj, acertos_obj_reg, lambda x: f"{x:.2f}%".replace(".", ","), nome_valor="Acertos")
    if part_obj_reg is not None:
        hover_part_reg_obj = hover_text_com_variacao(etapas_obj, part_obj_reg, lambda x: f"{x:.2f}%".replace(".", ","), nome_valor="Participação")
    ...
    fig_acertos.add_trace(
        go.Scatter(
            ...
            line=dict(color="#2ca02c"),
            hoverinfo="text",
            hovertext=hover_acertos_escola,
        )
    )
    ...
    fig_part.add_trace(
        go.Scatter(
            ...
            line=dict(color="#2ca02c"),
            hoverinfo="text",
            hovertext=hover_part_escola_obj,
        )
    )
